...
#include "S08_SEND_BYTE.pio.h"
#include "S08_READ_BYTE.pio.h"
#include "HCS08_BDM.h"
...
#define S08_CMD_READ_BYTE                   0xE0u
#define S08_CMD_READ_PROGRAM_PROGRAM_STATUS 0xE4u
#define S08_CMD_WRITE_BYTE                  0xC0u

#define SYS_SDIDH                           0x3002u
#define SYS_SDIDL                           0x3003u
#define NVM_FCLKDIV                         0x3020u
...
#define S08_PROGRAM_FLASH                   0x06u
#define S08_ERASE_ALL_BLOCK                 0x08u
#define S08_UNSECURE_NVM                    0x0Bu
#define S08_PROGRAM_EEPROM                  0x11u

#define NVM_FPROT_UNPROTECT_ALL             0xFFu
#define NVM_EEPROT_UNPROTECT_ALL            0x80u

#define NVM_CFG_RESERVED_BYTE               0xFFu
#define NVM_CFG_FPROT_BYTE                  0xFFu
#define NVM_CFG_EEPROT_BYTE                 0xFFu
#define NVM_CFG_FOPT_BYTE                   0xFFu
#define NVM_CFG_FSEC_UNSECURED              0xFEu
#define NVM_FSEC_BYTE_ADDRESS               0xFF7Fu
#define NVM_CFG_FIELD_START                 0xFF78u
#define NVM_CFG_FIELD_END                   0xFF7Fu
#define NVM_FSEC_SEC_MASK                   0x03u
#define NVM_FSEC_SEC_UNSECURED              0x02u

#define NVM_FSTAT_CCIF                      0x80u
#define NVM_FSTAT_ACCERR                    0x20u
#define NVM_FSTAT_FPVIOL                    0x10u
#define NVM_FSTAT_ERR_CLR                   0x30u
#define NVM_FSTAT_ERR_MASK                  0x33u

#define MC9S08PA4_DEV_ID                    0x0043u
#define MC9S08PA4_DEV_ID_MASK               0x0FFFu

#define NVM_FLASH_START                     0xF000u
#define NVM_FLASH_END                       0xFFFFu
#define NVM_FLASH_SIZE_BYTES                4096u
#define NVM_EEPROM_START                    0x3100u
#define NVM_EEPROM_END                      0x317Fu
#define NVM_EEPROM_SIZE_BYTES               128u
#define NVM_ERASED_BYTE                     0xFFu
#define NVM_FLASH_GLOBAL_HI                 0x00u
#define NVM_EEPROM_GLOBAL_HI                0x00u
#define NVM_FLASH_PHRASE_BYTES              8u
#define NVM_EEPROM_BURST_BYTES              4u

static uint8_t FCLKDIV;

static uint8_t FLASH_IMAGE[NVM_FLASH_SIZE_BYTES];
static uint8_t EEPROM_IMAGE[NVM_EEPROM_SIZE_BYTES];

static uint16_t BIT_TIME_SM_CYCLES;
static uint16_t TX_1_LOW_TIME_SM_CYCLES;
static uint16_t TX_1_HIGH_TIME_SM_CYCLES;
static uint16_t TX_0_LOW_TIME_SM_CYCLES;
static uint16_t TX_0_HIGH_TIME_SM_CYCLES;
static uint16_t RX_LOW_TIME_SM_CYCLES;
static uint16_t RX_WAIT_TO_SAMPLE_TIME_SM_CYCLES;
static uint16_t RX_FINISH_HIGH_TIME_SM_CYCLES;
static uint16_t RX_CMD_TO_DATA_DELAY_SM_CYCLES;

static const PIO  TX_BYTE_PIO = pio0;
static const uint TX_BYTE_SM  = 0;
static uint       TX_BYTE_OFFSET;

static const PIO  RX_BYTE_PIO = pio1;
static const uint RX_BYTE_SM  = 0;
static uint       RX_BYTE_OFFSET;
...
{
    TX_BYTE_OFFSET = pio_add_program(TX_BYTE_PIO, &S08_SEND_BYTE_program);
    RX_BYTE_OFFSET = pio_add_program(RX_BYTE_PIO, &S08_READ_BYTE_program);

    S08_SEND_BYTE_program_init(TX_BYTE_PIO, TX_BYTE_SM, TX_BYTE_OFFSET, PIN_BKGD, 32);
    S08_READ_BYTE_program_init(RX_BYTE_PIO, RX_BYTE_SM, RX_BYTE_OFFSET, PIN_BKGD);

    S08_SEND_BYTE_claim_pin(TX_BYTE_PIO, TX_BYTE_SM, TX_BYTE_OFFSET, PIN_BKGD);
}
...
static uint64_t S08_CMD_SYNC(void)
{
    ...
    {
        if (time_us_64() - START_TIMEOUT > 10000)
        {
    ...
    while (!gpio_get(PIN_BKGD))
    {
        if (time_us_64() - END_TIMEOUT > 10000)
        {
...
static inline void S08_TX_BYTE(PIO pio, uint sm, uint32_t LOW_CYCLES, uint32_t HIGH_CYCLES, uint8_t CMD, uint16_t ADDRESS, uint8_t DATA)
{
    ...
    S08_SEND_BYTE_load_timing(pio, sm, LOW_CYCLES, HIGH_CYCLES);
    S08_SEND_BYTE_send(pio, sm, PACKET, 32);
}
...
static inline uint8_t S08_RX_BYTE(PIO pio, uint sm, uint32_t LOW_CYCLES, uint32_t HIGH_CYCLES, uint8_t CMD, uint16_t ADDRESS)
{
    ...
    S08_SEND_BYTE_load_timing(pio, sm, LOW_CYCLES, HIGH_CYCLES);
    S08_SEND_BYTE_send(pio, sm, PACKET, 24);

    S08_SEND_BYTE_wait_idle(pio, sm);
    pio_sm_set_enabled(pio, sm, false);

    S08_READ_BYTE_claim_pin(RX_BYTE_PIO, RX_BYTE_SM, RX_BYTE_OFFSET, PIN_BKGD);

    uint8_t DATA = S08_READ_BYTE_receive(RX_BYTE_PIO, RX_BYTE_SM,
    ...
    S08_SEND_BYTE_claim_pin(pio, sm, TX_BYTE_OFFSET, PIN_BKGD);
 ...
 * INPUT:       S19 Staging Buffer, # of packets
...
bool PROGRAM_MC9S08PA4(const S19Packet_t *BUFFER, size_t TOTAL_PACKETS)
{
    ...
    uint16_t BDC_TIMING_NS = (uint16_t)(((uint32_t)BDC_CLK_NS * 108u + 50u) / 100u);

    BIT_TIME_SM_CYCLES               = S08_CONVERT_TO_SM_CYCLES(16, BDC_TIMING_NS, SM_MHZ);
    TX_1_LOW_TIME_SM_CYCLES          = S08_CONVERT_TO_SM_CYCLES(3, BDC_TIMING_NS, SM_MHZ);
    TX_1_HIGH_TIME_SM_CYCLES         = S08_CONVERT_TO_SM_CYCLES(13, BDC_TIMING_NS, SM_MHZ);
    TX_0_LOW_TIME_SM_CYCLES          = TX_1_HIGH_TIME_SM_CYCLES;
    TX_0_HIGH_TIME_SM_CYCLES         = TX_1_LOW_TIME_SM_CYCLES;
    RX_LOW_TIME_SM_CYCLES            = TX_1_LOW_TIME_SM_CYCLES - 1;
    RX_WAIT_TO_SAMPLE_TIME_SM_CYCLES = S08_CONVERT_TO_SM_CYCLES(7, BDC_TIMING_NS, SM_MHZ);
    RX_FINISH_HIGH_TIME_SM_CYCLES    = RX_WAIT_TO_SAMPLE_TIME_SM_CYCLES;
    RX_CMD_TO_DATA_DELAY_SM_CYCLES   = BIT_TIME_SM_CYCLES;

    if (BDC_CLK_NS < 50 || BDC_CLK_NS > 1000)
    {
        restore_interrupts(IRQ_PROGRAM_STATUS);
        S08_POWER_CYCLE();
        return false;
    }
    else if (BDC_CLK_NS <= 51)
    {
        FCLKDIV = 0x13;
    }
    else if (BDC_CLK_NS <= 53)
    {
        FCLKDIV = 0x12;
    }
    else if (BDC_CLK_NS <= 56)
    {
        FCLKDIV = 0x11;
    }
    else if (BDC_CLK_NS <= 60)
    {
        FCLKDIV = 0x10;
    }
    else if (BDC_CLK_NS <= 64)
    {
        FCLKDIV = 0x0F;
    }
    else if (BDC_CLK_NS <= 68)
    {
        FCLKDIV = 0x0E;
    }
    else if (BDC_CLK_NS <= 73)
    {
        FCLKDIV = 0x0D;
    }
    else if (BDC_CLK_NS <= 79)
    {
        FCLKDIV = 0x0C;
    }
    else if (BDC_CLK_NS <= 86)
    {
        FCLKDIV = 0x0B;
    }
    else if (BDC_CLK_NS <= 94)
    {
        FCLKDIV = 0x0A;
    }
    else if (BDC_CLK_NS <= 104)
    {
        FCLKDIV = 0x09;
    }
    else if (BDC_CLK_NS <= 116)
    {
        FCLKDIV = 0x08;
    }
    else if (BDC_CLK_NS <= 131)
    {
        FCLKDIV = 0x07;
    }
    else if (BDC_CLK_NS <= 151)
    {
        FCLKDIV = 0x06;
    }
    else if (BDC_CLK_NS <= 178)
    {
        FCLKDIV = 0x05;
    }
    else if (BDC_CLK_NS <= 217)
    {
        FCLKDIV = 0x04;
    }
    else if (BDC_CLK_NS <= 277)
    {
        FCLKDIV = 0x03;
    }
    else if (BDC_CLK_NS <= 384)
    {
        FCLKDIV = 0x02;
    }
    else if (BDC_CLK_NS <= 625)
    {
        FCLKDIV = 0x01;
    }
    else
    ...
    for (size_t PACKET = 0; PACKET < TOTAL_PACKETS; PACKET++)
    {
        ...
        for (uint16_t OFFSET = 0; OFFSET < S19_PAYLOAD_SIZE_BYES; OFFSET++)
        {
            ...
            if ((TARGET >= NVM_FLASH_START) && (TARGET <= NVM_FLASH_END))
            {
                ...
                {
                    if ((uint16_t)TARGET < LOWEST_FLASH)
                    {
                        LOWEST_FLASH = (uint16_t)TARGET;
                    }
                    if ((uint16_t)TARGET > HIGHEST_FLASH)
                    {
                        HIGHEST_FLASH = (uint16_t)TARGET;
                    }
                }
            }
